...
export function UploadZone({ initialPrefs }: UploadZoneProps) {
    ...
    const handleAnalyze = async () => {
        ...
        try {
            ...
            alert(messages.upload.errorMessage)
            setStatus('idle')
        }
    }
                ...
                {status !== 'idle' ? (
                    <div className='flex flex-col items-center gap-4 py-20 px-6'>
                        ...
                        <p className='text-xl font-bold text-slate-900'>
                            {status === 'compressing' ? messages.upload.compressingTitle : messages.upload.analyzingTitle}
                        </p>
                        <p className='text-slate-500'>
                            {status === 'compressing' ? messages.upload.compressingSubtitle : messages.upload.analyzingSubtitle}
                        </p>
                    </div>
                ) : file ? (
